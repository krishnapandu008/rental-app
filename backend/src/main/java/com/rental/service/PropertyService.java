...
public class PropertyService {
	...
	public Page<PropertyResponseDto> getVisibleProperties(Long ownerId, String role, String location, Double minPrice,
		...
		try {
			...
			log.error(
					"Error fetching properties with filters: ownerId={}, role={}, location={}, minPrice={}, maxPrice={}, bedrooms={}, pageable={}",
					ownerId, role, location, minPrice, maxPrice, bedrooms, pageable, e);
			throw e;
		}
	}
	...
	public PropertyResponseDto create(PropertyRequestDto dto, Long authenticatedOwnerId, List<MultipartFile> images) {
		Property property = Property.builder().title(dto.getTitle()).description(dto.getDescription())
				...
				.contactNumber(dto.getContactNumber()).ownerId(authenticatedOwnerId)
				.available(dto.getAvailable() != null ? dto.getAvailable() : true)
				.visibility(dto.getVisibility() != null ? dto.getVisibility() : Visibility.PUBLIC).isActive(true)
				.createdAt(LocalDateTime.now()).latitude(dto.getLatitude()).longitude(dto.getLongitude()).build();
